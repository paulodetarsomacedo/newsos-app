...
function FeedTab({ openArticle, isDarkMode, selectedArticleId, savedItems, onToggleSave, readHistory, newsData, isLoading }) {
  ...
  // Se newsData (dados reais) existir e não estiver vazio, use-o. Senão, use FEED_NEWS (mock).
  const safeNews = (newsData && newsData.length > 0) ? newsData : FEED_NEWS;

  // Lógica de Filtragem (CORRIGIDA: Declarada apenas uma vez)
  ...
  const handleTouchEnd = async () => {
    ...
    if (pullDistance > 70) {
        ...
        setPullDistance(70); 
        // Aqui você poderia chamar uma função de recarregar real passada via props
    ...
    >
      ...
      <div className="sticky top-0 z-[1000] w-full flex justify-center py-2 pointer-events-none">
          <div className="pointer-events-auto">
             {/* Passamos safeNews para o seletor saber quais fontes existem de verdade */}
      ...
      <div className="flex flex-col gap-4">
        ...
        {displayedNews.map((news, index) => {
          ...
          const isRead = readHistory?.includes(news.id);
...
// Lista de proxies para tentar em ordem (Estratégia de Rotação)
const PROXY_LIST = [
    // 1. CorsProxy.io (Mais rápido, mas bloqueia fácil)
    (url) => `https://corsproxy.io/?${encodeURIComponent(url)}`,
    
    // 2. AllOrigins (Muito estável, retorna o XML puro)
    (url) => `https://api.allorigins.win/raw?url=${encodeURIComponent(url)}`,
    
    // 3. CodeTabs (Ótimo fallback quando os outros falham)
    (url) => `https://api.codetabs.com/v1/proxy/?quest=${encodeURIComponent(url)}`,
];

// Função auxiliar para tentar baixar o RSS usando a lista acima
const fetchWithRetry = async (url) => {
    for (const createProxyUrl of PROXY_LIST) {
        try {
            const targetUrl = createProxyUrl(url);
            // Adicionamos headers para tentar evitar bloqueios de bot
            ...
            // Validação simples: se não começar com <, provavelmente não é XML válido
            if (!text.trim().startsWith('<')) throw new Error('Resposta não parece XML');
            
            return text; 
        } catch (error) {
            console.warn(`Tentativa falhou para ${url} usando proxy. Tentando próximo...`);
            continue; // Tenta o próximo proxy da lista
        }
    }
 ...
 const fetchFeeds = async () => {
    ...
    const promises = userFeeds.map(async (feed) => {
        // Se já deu erro antes ou está desativado, pula
        if (!feed.display?.feed && !feed.display?.banca) return;
        
        // Pula se não tiver URL válida
        if (!feed.url) return;
        ...
        try {
            ...
            // BLOQUEIO DO LOOP:
            // Se o backend retornar erro, nós LOGAMOS e RETORNAMOS.
            // Não jogamos erro (throw) para não quebrar o Promise.all
            if (error || (data && data.error)) {
                console.warn(`🚫 Feed ignorado por erro (${feed.name}):`, error || data.error);
                return; // <--- O PULO DO GATO: Simplesmente sai da função para este feed
            }

            if (!data || !data.items) return;

            // Se chegou aqui, funcionou. Continua a lógica normal...
            if (feed.name === 'Nova Fonte' || feed.name === 'Sem Título') {
                feedsThatNeedUpdate.push({
                    id: feed.id,
                    name: data.title, 
                });
            }
    ...
    uniqueItems.sort((a, b) => b.rawDate - a.rawDate);
  ...
  const showNavBar = () => { if (!isNavVisible) setIsNavVisible(true); };
  ...
  return (
    <div className={`min-h-[100dvh] font-sans overflow-hidden selection:bg-blue-500/30 transition-colors duration-500 ${isDarkMode ? 'bg-slate-900 text-zinc-100' : 'bg-slate-100 text-zinc-900'}`}>      
      ...
      <div className={`transition-all duration-500 transform h-[100dvh] flex flex-col ${isMainViewReceded ? `scale-[0.9] pointer-events-none` : 'scale-100 opacity-100'}`}>
          ...
          <HeaderDashboard isDarkMode={isDarkMode} onToggleTheme={() => setIsDarkMode(!isDarkMode)} onOpenSettings={() => setIsSettingsOpen(true)} />
...
function ArticlePanel({ article, feedItems, isOpen, onClose, onArticleChange, onToggleSave, isSaved, isDarkMode }) {
  ...
  const [htmlContent, setHtmlContent] = useState('');     // HTML para o Iframe
  const [readerContent, setReaderContent] = useState(null); // Conteúdo limpo para leitura
  ...
  const baseClasses = `fixed inset-0 z-[5000] flex flex-col bg-white transition-transform duration-300 ease-out ${isDarkMode ? 'bg-zinc-950' : 'bg-white'}`;
  ...
  // Componente do Navegador Flutuante (mantido igual)
  const FeedNavigator = () => {
      if (!isOpen) return null;
  ...
  return (
    <div className={`${baseClasses} ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        {/* --- ALÇA DE REDIMENSIONAMENTO (CORRIGIDA E VISÍVEL) --- */}
        {/* Fica visível apenas em telas maiores (desktop) */}
        {!isStoryMode && (
             <div 
            ...
            >
                ...
                <div className={`h-24 w-1.5 rounded-full flex items-center justify-center transition-all duration-300 
                    ${isDragging 
                        ? 'bg-blue-500 scale-y-110 shadow-lg' // Estado ativo
                        : 'bg-zinc-200 dark:bg-zinc-800 group-hover:bg-zinc-300 dark:group-hover:bg-zinc-700' // Estado normal/hover
                    }`}
                >
                    <GripVertical size={16} className={`transition-opacity duration-300 
                        ${isDragging 
                            ? 'text-white' 
        ...
        {/* --- PAINEL PRINCIPAL (ajustado para a alça) --- */}
        <div className="relative flex-1 w-full flex flex-col md:pl-2">
            ...
            {/* ÁREA DE CONTEÚDO */}
            <div className="flex-1 relative w-full h-full overflow-hidden bg-zinc-100 dark:bg-zinc-900" style={{ width: `calc(100% - ${isDragging ? 0 : 0}px)` }}>
                {/* (Renderização condicional dos modos Web, Reader, AI mantida igual) */}
...
function SettingsModal({ onClose, isDarkMode, feeds, setFeeds, apiKey, setApiKey }) {
  ...
  // FUNÇÃO CORRIGIDA
  const handleAddFeed = () => {
    if (!newUrl.trim()) return; // Ignora se o campo estiver vazio
    ...
    // --- INÍCIO DA CORREÇÃO ---
    // 1. Normaliza a URL para garantir que tenha um protocolo.
    let formattedUrl = newUrl.trim();
    if (!formattedUrl.startsWith('http://') && !formattedUrl.startsWith('https://')) {
      // Adiciona https:// por padrão, que é mais seguro e comum.
    ...
    const newFeed = { 
        id: Date.now(), 
        name: 'Nova Fonte', // Supabase vai tentar descobrir o nome real depois
        url: formattedUrl, // 2. Usa a URL formatada
            ...
            {activeTab === 'sources' && (
                <div className="space-y-6">
                    ...
                    <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-zinc-800/50 border-white/5' : 'bg-zinc-50 border-zinc-200'}`}>
                        ...
                        <input type="text" value={newUrl} onChange={(e) => setNewUrl(e.target.value)} placeholder="ex: cidadeverde.com/rss" className={`w-full mb-4 px-3 py-2 rounded-lg text-sm outline-none border transition-all ${isDarkMode ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-300'}`} />
