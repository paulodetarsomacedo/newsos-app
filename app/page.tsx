...
const GlassBrowser = ({ article, onClose, isDarkMode, onFetchContent, onAnalyze }) => { // onAnalyze = abre painel lateral / chat WhatsApp
  ...
  const smartSummary = useMemo(() => {
    // ───────────────────────────────────────────────────────────
  // DIAGNÓSTICO TEMPORÁRIO — remover depois de colher o output.
  // Loga o que o article carrega e o que chega do proxy-lite.
  // ───────────────────────────────────────────────────────────
  useEffect(() => {
    if (!article) return;
    const val = (v: any) =>
      v == null ? 'undefined'
      : Array.isArray(v) ? `array[${v.length}]${v.length ? ' → ' + JSON.stringify(v.slice(0, 4)) : ''}`
      : typeof v === 'string' ? `string(${v.length}) "${v.slice(0, 60)}${v.length > 60 ? '…' : ''}"`
      : typeof v === 'object' ? `object{${Object.keys(v).slice(0, 8).join(',')}}`
      : `${typeof v}: ${v}`;

    console.log(
      '%c🔍 VETRA DIAG — article recebido pelo GlassBrowser',
      'background:#0b1733;color:#7dd3fc;padding:2px 8px;border-radius:4px;font-weight:bold'
    );
    console.log('  ▸ TODAS as chaves:', Object.keys(article));
    console.table({
      title:            { valor: val(article.title) },
      source:           { valor: val(article.source) },
      category:         { valor: val(article.category) },
      summary:          { valor: val(article.summary ?? article.description) },
      // — campos enriquecidos (Patch 3): se vierem 'undefined', estão bloqueados —
      contextText:      { valor: val(article.contextText) },
      contextLevel:     { valor: val(article.contextLevel) },
      entities:         { valor: val(article.entities) },
      keyphrases:       { valor: val(article.keyphrases) },
      eventType:        { valor: val(article.eventType) },
      normalizedTitle:  { valor: val(article.normalizedTitle) },
      coreWords:        { valor: val(article.coreWords) },
      contentHash:      { valor: val(article.contentHash) },
      // — datas —
      rawDate:          { valor: val(article.rawDate) },
      publishedAt:      { valor: val(article.publishedAt) },
    });

    // Veredito rápido sobre o Patch 3
    const enriched = ['contextText', 'entities', 'keyphrases', 'eventType', 'coreWords', 'normalizedTitle'];
    const presentes = enriched.filter(k => article[k] != null && (Array.isArray(article[k]) ? article[k].length : true));
    console.log(
      `  ▸ CAMPOS ENRIQUECIDOS PRESENTES: ${presentes.length}/${enriched.length}` +
      (presentes.length === 0
        ? '  ❌ Patch 3 provavelmente NÃO aplicado — motor está cego dos campos enriquecidos.'
        : presentes.length < enriched.length
          ? `  ⚠️ parcial → ${presentes.join(', ')}`
          : '  ✅ todos chegando.'),
      presentes.length === 0 ? 'color:#f87171;font-weight:bold' : 'color:#fbbf24'
    );
  }, [article]);

  // Loga o que o proxy-lite trouxe (extractedText) quando chega/atualiza.
  useEffect(() => {
    if (content == null) return;
    const c = String(content);
    console.log(
      '%c🔍 VETRA DIAG — extractedText (proxy-lite) chegou',
      'background:#0b1733;color:#86efac;padding:2px 8px;border-radius:4px;font-weight:bold'
    );
    console.log(`  ▸ length: ${c.length}`);
    console.log(`  ▸ início: "${c.slice(0, 200).replace(/\s+/g, ' ')}${c.length > 200 ? '…' : ''}"`);
    // Detecta ruído bruto (menu/timestamp/crédito colado) ANTES do meu saneador.
    const suspeito = /(Menu|Cadastre-se|Todos os campeonatos|Getty Images|Atualizado em|\d{2}\/\d{2}\/\d{4})/i.test(c);
    console.log(`  ▸ contém ruído bruto (menu/data/crédito)? ${suspeito ? '⚠️ SIM' : '✅ não aparenta'}`);
  }, [content]);
  // ─────────────────── fim do diagnóstico ───────────────────
    try {
