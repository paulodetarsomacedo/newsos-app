...
export interface NormalizedArticle {
  ...
  _domain: string;         // domínio cacheado (evita recalcular por par)
}
...
export const normalizeArticleForCluster = (
  ...
  try {
    ...
    return {
      ...
      _domain: getDomainHints(title)[0] || 'geral',
    };
  } catch {
...
const pickDomainWeights = (a: NormalizedArticle, b: NormalizedArticle): WeightSet => {
  const domain = (a._domain === b._domain) ? a._domain : (a._domain !== 'geral' ? a._domain : b._domain);
...
export const generateSmartHeuristicClusters = (articles: any[], options: ClusterEngineOptions = {}) => {
  ...
  // ==========================================================================
  // 2) AGRUPAMENTO AGLOMERATIVO COM FUSÃO — versão eficiente
  // A matriz de similaridade é calculada UMA VEZ (O(n²)). As fusões consultam
  // o cache, em vez de recomparar os artigos a cada iteração (isso era O(n³)
  // e travava a UI por >1s com 200 artigos).
  // ==========================================================================
  ...
  for (let i = 0; i < normalized.length; i++) {
    for (let j = i + 1; j < normalized.length; j++) {
      ...
      if (!withinCaseWindow(a, b)) continue;
      if (stringSimilarity.compareTwoStrings(a.cleanTitle, b.cleanTitle) >= cfg.duplicateTitleSim) {
        dupOf.add(b.id);
      }
    }
  }
  const pool = normalized.filter(a => !dupOf.has(a.id));
  const N = pool.length;
  const idxOf = new Map<any, number>();
  pool.forEach((a, i) => idxOf.set(a.id, i));

  // Matriz de pares (uma única passada). Fora da janela do caso → -1 (proibido).
  const pairSim: Float32Array = new Float32Array(N * N).fill(-1);
  for (let i = 0; i < N; i++) {
    for (let j = i + 1; j < N; j++) {
      const a = pool[i], b = pool[j];
      if (!withinCaseWindow(a, b)) continue;              // filtro duro (peça 4)
  ...
  // Cada artigo começa como seu próprio cluster (guardamos índices).
  let groups: number[][] = pool.map((_, i) => [i]);

  // Similaridade entre clusters: híbrido average + piso pelo melhor par.
  // - AVERAGE sozinho é conservador: uma matéria de título atípico ("Caso Master:
  //   PF alega risco de fuga...") tem média baixa e ficaria de fora do seu caso.
  // - MAX sozinho encadeia clusters distintos (basta um par parecido).
  const simOf = (A: number[], B: number[]): number => {
    let sum = 0, n = 0, best = -1;
    for (const i of A) {
      for (const j of B) {
        const s = pairSim[i * N + j];
        if (s < 0) return -1;          // par fora da janela → clusters incompatíveis
        sum += s;
        if (s > best) best = s;
        n++;
      }
    ...
    if (n === 0) return -1;
    return Math.max(sum / n, best * 0.82);
  };

  // Fusão iterativa: une sempre o par de clusters mais similar acima do limiar.
  // (peça 5) SEM limite por fonte — descartar matéria PERDE cobertura. A
  // diversidade é medida por GRUPOS editoriais, não jogando conteúdo fora.
  let merged = true;
  let guard = 0;
  while (merged && groups.length > 1 && guard++ < 500) {
    merged = false;
    let bestI = -1, bestJ = -1, bestScore = cfg.scoreThreshold;

    for (let i = 0; i < groups.length; i++) {
      for (let j = i + 1; j < groups.length; j++) {
        const s = simOf(groups[i], groups[j]);
